...
router.get("/dashboard/summary", requireAuth, async (_req, res): Promise<void> => {
  ...
  const [todayRes, weeklyRes, monthlyRes, totalMedRes, lowStockRes, expiringRes, expiredRes, invValueRes] =
    await Promise.all([
      db.execute(sql`SELECT COALESCE(SUM(total_amount),0) as sales, COALESCE(SUM(total_amount)-SUM(discount_amount),0) as profit FROM sales WHERE sale_date = ${today}`),
      db.execute(sql`SELECT COALESCE(SUM(total_amount),0) as sales FROM sales WHERE sale_date >= ${weekAgo}`),
      db.execute(sql`SELECT COALESCE(SUM(total_amount),0) as sales FROM sales WHERE sale_date >= ${monthAgo}`),
      db.execute(sql`SELECT COUNT(*) as count FROM medicines`),
      db.execute(sql`SELECT COUNT(*) as count FROM medicines WHERE quantity <= reorder_level`),
      db.execute(sql`SELECT COUNT(*) as count FROM medicines WHERE expiry_date >= ${today} AND expiry_date <= ${in30}`),
      db.execute(sql`SELECT COUNT(*) as count FROM medicines WHERE expiry_date < ${today}`),
      db.execute(sql`SELECT COALESCE(SUM(quantity * purchase_price),0) as value FROM medicines`),
    ]);
...
router.get("/dashboard/low-stock", requireAuth, async (_req, res): Promise<void> => {
  const result = await db.execute(sql`
    ...
    FROM medicines WHERE quantity <= reorder_level ORDER BY quantity ASC
  `);
  const rows = (result as any).rows ?? result;
  res.json(rows.map((r: any) => ({
    id:           parseInt(r.id),
